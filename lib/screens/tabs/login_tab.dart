  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // ✅ ALERTAS DE INICIO DE SESIÓN EXITOSO
        // 1. Overlay arriba (pequeña) - aparece primero
        ...
        // Esperar 100ms antes de mostrar la segunda alerta
        await Future.delayed(const Duration(milliseconds: 100));

        // 2. SnackBar abajo (grande) - aparece después
        if (mounted) {
          if (cliente.esInstalador) {
            CustomSnackBar.showSuccess(
              context,
              'Bienvenido Admin - ${cliente.nombre}',
            );
          } else {
            CustomSnackBar.showSuccess(
              context,
              'Sesión iniciada como ${cliente.nombre}',
            );
          }
        }

        // Delay antes de cambiar de pantalla
        await Future.delayed(const Duration(milliseconds: 400));
        widget.onLoginSuccess(cliente);
      }
    } catch (e) {
      setState(() {
